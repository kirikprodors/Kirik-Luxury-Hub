...
local playerColors = {} -- Stores custom colors for players

local function GetPlayerColor(player)
    return playerColors[player.Name] or (currentTheme == "HACKER" and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(0, 255, 255))
end
...
local function ApplyStyle(inst, strokeColor, bgColor, textColor)
    ...
    if inst:IsA("TextButton") then
        inst.MouseEnter:Connect(function() TweenService:Create(stroke, TweenInfo.new(0.2), {Thickness = 2.5}):Play() end)
        inst.MouseLeave:Connect(function() TweenService:Create(stroke, TweenInfo.new(0.2), {Thickness = 1.5}):Play() end)
    end
...
local function B64Encode(data) return ((data:gsub('.', function(x) local r, b = '', x:byte() for i = 8, 1, -1 do r = r .. (b % 2^i - b % 2^(i-1) > 0 and '1' or '0') end return r; end)..'0000'):gsub('%d%d%d?%d?%d?%d?', function(x) if (#x < 6) then return '' end local c = 0 for i = 1, 6 do c = c + (x:sub(i,i) == '1' and 2^(6-i) or 0) end return b64chars:sub(c+1, c+1) end)..({ '', '==', '=' })[#data%3+1]) end
local function B64Decode(data) data = string.gsub(data, '[^'..b64chars..'=]', '') return (data:gsub('.', function(x) if (x == '=') then return '' end local r, f = '', (b64chars:find(x)-1) for i = 6, 1, -1 do r = r .. (f % 2^i - f % 2^(i-1) > 0 and '1' or '0') end return r; end):gsub('%d%d%d?%d?%d?%d?%d?%d?', function(x) if (#x ~= 8) then return '' end local c = 0 for i = 1, 8 do c = c + (x:sub(i,i) == '1' and 2^(8-i) or 0) end return string.char(c) end)) end
...
StatsLayout.Padding = UDim.new(0, 4) StatsLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center StatsLayout.VerticalAlignment = Enum.VerticalAlignment.Center
local PingLbl = Instance.new("TextLabel", StatsFrame) PingLbl.Size = UDim2.new(1, -10, 0.45, 0) PingLbl.BackgroundTransparency = 1 PingLbl.Visible = false ApplyStyle(PingLbl, Color3.fromRGB(255, 150, 0))
local FpsLbl = Instance.new("TextLabel", StatsFrame) FpsLbl.Size = UDim2.new(1, -10, 0.45, 0) FpsLbl.BackgroundTransparency = 1 FpsLbl.Visible = false ApplyStyle(FpsLbl, Color3.fromRGB(0, 255, 100))
...
ApplyStyle(RadarFrame, Color3.fromRGB(0, 255, 255), Color3.fromRGB(10, 5, 15))
Instance.new("UICorner", RadarFrame).CornerRadius = UDim.new(0.5, 0) -- Circle
local RadarCenter = Instance.new("Frame", RadarFrame)
RadarCenter.Size = UDim2.new(0, 4, 0, 4) RadarCenter.Position = UDim2.new(0.5, -2, 0.5, -2) RadarCenter.BackgroundColor3 = Color3.new(1, 1, 1) Instance.new("UICorner", RadarCenter).CornerRadius = UDim.new(1, 0)
local RadarBlips = Instance.new("Folder", RadarFrame)

-- ESP FOLDER
local ESPFolder = Instance.new("Folder", ScreenGui)
ESPFolder.Name = "ESPDistances"
...
MinBtn.Text = "-" MinBtn.Size = UDim2.new(0, 20, 0, 20) MinBtn.Position = UDim2.new(1, -50, 0, 3) ApplyStyle(MinBtn, Color3.fromRGB(255, 255, 0))

local CloseBtn = Instance.new("TextButton", MainFrame)
CloseBtn.Text = "X" CloseBtn.Size = UDim2.new(0, 20, 0, 20) CloseBtn.Position = UDim2.new(1, -25, 0, 3) ApplyStyle(CloseBtn, Color3.fromRGB(255, 0, 0))

-- TAB SYSTEM
local Sidebar = Instance.new("Frame", MainFrame) Sidebar.Size = UDim2.new(0, 110, 1, -35) Sidebar.Position = UDim2.new(0, 5, 0, 30) Sidebar.BackgroundTransparency = 1
...
local SearchBox = Instance.new("TextBox", Sidebar) SearchBox.Size = UDim2.new(1, 0, 0, 25) SearchBox.PlaceholderText = "SEARCH..." SearchBox.LayoutOrder = -1
ApplyStyle(SearchBox, Color3.fromRGB(255, 255, 255), Color3.fromRGB(20, 20, 30))

local TabContainer = Instance.new("Frame", MainFrame) TabContainer.Size = UDim2.new(1, -125, 1, -35) TabContainer.Position = UDim2.new(0, 120, 0, 30) TabContainer.BackgroundTransparency = 1
...
local function MakeTab(name, isDefault)
    ...
    btn.MouseButton1Click:Connect(function()
        for i, t in ipairs(tabs) do
            if t.Visible then TweenService:Create(t, TweenInfo.new(0.2), {GroupTransparency = 1}):Play() task.delay(0.2, function() t.Visible = false end) end
        end
        ...
        page.GroupTransparency = 1 page.Visible = true TweenService:Create(page, TweenInfo.new(0.2), {GroupTransparency = 0}):Play()
        btn:SetAttribute("NeonStroke", Color3.fromRGB(255, 0, 255)) btn:SetAttribute("NeonBg", Color3.fromRGB(30, 20, 40)) UpdateInstanceTheme(btn)
    end)
...
local function MakeScrollArea(parent)
    local scroll = Instance.new("ScrollingFrame", parent) scroll.Size = UDim2.new(1, 0, 1, 0) scroll.BackgroundTransparency = 1 scroll.ScrollBarThickness = 3 scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    if minimized then
        Sidebar.Visible = false TabContainer.Visible = false SearchBox.Visible = false Title.Visible = false
    end
    local tween = TweenService:Create(MainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = targetSize})
    tween:Play()
    tween.Completed:Connect(function()
        if not minimized then Sidebar.Visible = true TabContainer.Visible = true SearchBox.Visible = true Title.Visible = true end
    end)
end)

-- GLOBAL STATES & SETTERS
local espActive, ultraRunActive, noclipActive, invisActive, undieActive, platActive, unvoidActive = false, false, false, false, false, false, false
local flying, infStabActive, cfSpeedActive, spinActive, fpsActive, pingActive, aimbotActive, radarActive, unafkActive = false, false, false, false, false, false, false, false, false
local updateLagList, updatePlayerList, updateNpcList, PerformSearch
local ToggleInvis, TogglePlatform, SetFly

-- ==================== TABS CREATION ====================

-- 1. HOME TAB
local HomeTab = MakeTab("HOME", true)
local WelcomeText = Instance.new("TextLabel", HomeTab) WelcomeText.Size = UDim2.new(1, 0, 1, 0)
WelcomeText.Text = "KIRIK HUB V50\n\n[ HIGHLIGHTS ]\n- ESP+ (Distance) & Aimbot\n- Radar Map & Live Search\n- Replay System & UN-AFK\n- Smooth Animations\n- Ghost Invisibility & Auto-Save"
WelcomeText.TextWrapped = true WelcomeText.TextYAlignment = Enum.TextYAlignment.Top ApplyStyle(WelcomeText, Color3.fromRGB(0, 255, 255), Color3.fromRGB(15, 15, 20))
...
-- 3. COMBAT TAB
local CombatTab = MakeTab("COMBAT", false)
local CombatScroll, _ = MakeScrollArea(CombatTab)
local AimbotBtn = Instance.new("TextButton", MakeRow(CombatScroll)) AimbotBtn.Size = UDim2.new(1, 0, 1, 0) AimbotBtn.Text = "AIMBOT (HOLD RMB): OFF" ApplyStyle(AimbotBtn, Color3.fromRGB(255, 0, 50))
local RadarBtn = Instance.new("TextButton", MakeRow(CombatScroll)) RadarBtn.Size = UDim2.new(1, 0, 1, 0) RadarBtn.Text = "RADAR MAP: OFF" ApplyStyle(RadarBtn, Color3.fromRGB(0, 255, 255))
...
local RecBtn = Instance.new("TextButton", MakeRow(ReplayTab)) RecBtn.Size = UDim2.new(1, 0, 1, 0) RecBtn.Text = "RECORD REPLAY: OFF" ApplyStyle(RecBtn, Color3.fromRGB(255, 50, 50))
local PlayRecBtn = Instance.new("TextButton", MakeRow(ReplayTab)) PlayRecBtn.Size = UDim2.new(1, 0, 1, 0) PlayRecBtn.Text = "WATCH REPLAY: OFF" ApplyStyle(PlayRecBtn, Color3.fromRGB(0, 255, 50))
local ClearRecBtn = Instance.new("TextButton", MakeRow(ReplayTab)) ClearRecBtn.Size = UDim2.new(1, 0, 1, 0) ClearRecBtn.Text = "CLEAR RECORDING" ApplyStyle(ClearRecBtn, Color3.fromRGB(255, 150, 0))
...
local ShrinkLbl = Instance.new("TextLabel", ShrinkRow) ShrinkLbl.Size = UDim2.new(0.65, 0, 1, 0) ShrinkLbl.Text = "SHRINK UI (Ex: 2)" ApplyStyle(ShrinkLbl, Color3.fromRGB(255, 255, 0))
...
local UnAfkBtn = Instance.new("TextButton", MakeRow(SettingsScroll)) UnAfkBtn.Parent.LayoutOrder = 2
UnAfkBtn.Size = UDim2.new(1, 0, 1, 0) UnAfkBtn.Text = "UN-AFK (ANTI-IDLE): OFF" ApplyStyle(UnAfkBtn, Color3.fromRGB(150, 150, 255))
...
PerformSearch = function()
    ...
    for i, tBtn in ipairs(tabBtns) do
        ...
        local scrolls = {}
        for _, obj in ipairs(tabObj:GetDescendants()) do if obj:IsA("ScrollingFrame") then table.insert(scrolls, obj) end end
        if #scrolls == 0 then table.insert(scrolls, tabObj) end
        
        for _, container in ipairs(scrolls) do
            for _, row in ipairs(container:GetChildren()) do
                if row:IsA("Frame") and row.Name ~= "DragHandle" then
        ...
        tBtn.Visible = (q == "" or tabMatches or hasVisibleRow)
    end
end
...
local function SetUnAFK(state) unafkActive = state ApplyToggleStyle(UnAfkBtn, unafkActive, Color3.fromRGB(150, 150, 255)) UnAfkBtn.Text = "UN-AFK (ANTI-IDLE): " .. (unafkActive and "ON" or "OFF") end
UnAfkBtn.MouseButton1Click:Connect(function() SetUnAFK(not unafkActive) end)

LocalPlayer.Idled:Connect(function() if unafkActive then VirtualUser:CaptureController() VirtualUser:ClickButton2(Vector2.new()) end end)
...
-- RADAR LOGIC
local function SetRadar(state)
    radarActive = state
    ApplyToggleStyle(RadarBtn, radarActive, Color3.fromRGB(0, 255, 255))
    RadarBtn.Text = "RADAR MAP: " .. (radarActive and "ON" or "OFF")
    RadarFrame.Visible = radarActive
end
...
RunService.RenderStepped:Connect(function()
    ...
    RadarBlips:ClearAllChildren()
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            local tHrp = p.Character.HumanoidRootPart
            local rel = hrp.CFrame:PointToObjectSpace(tHrp.Position)
            local dist = math.sqrt(rel.X^2 + rel.Z^2)
            if dist <= 150 then
                local dot = Instance.new("Frame", RadarBlips)
                dot.Size = UDim2.new(0, 4, 0, 4)
                dot.BackgroundColor3 = GetPlayerColor(p)
                Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
                dot.Position = UDim2.new(0.5, (rel.X/150)*75 - 2, 0.5, (rel.Z/150)*75 - 2)
            end
        end
    end
end)
...
local function SetAimbot(state)
    aimbotActive = state
    ApplyToggleStyle(AimbotBtn, aimbotActive, Color3.fromRGB(255, 0, 50))
    AimbotBtn.Text = "AIMBOT (HOLD RMB): " .. (aimbotActive and "ON" or "OFF")
end
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Head") then
                ...
                if vis then
                    ...
                    if dist < shortest then shortest = dist nearest = p.Character.Head end
                end
            end
        end
        if nearest then TweenService:Create(cam, TweenInfo.new(0.05), {CFrame = CFrame.new(cam.CFrame.Position, nearest.Position)}):Play() end
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            local tHrp = p.Character.HumanoidRootPart
            local c = GetPlayerColor(p)
            
            local hl = Instance.new("Highlight", ESPFolder)
            hl.Adornee = p.Character hl.FillColor = c hl.OutlineColor = c hl.FillTransparency = 0.5
            
            if myHrp then
                local dist = math.floor((myHrp.Position - tHrp.Position).Magnitude)
                local bg = Instance.new("BillboardGui", ESPFolder)
                bg.Adornee = p.Character:FindFirstChild("Head") or tHrp
                bg.Size = UDim2.new(0, 200, 0, 50) bg.StudsOffset = Vector3.new(0, 2, 0) bg.AlwaysOnTop = true
                local txt = Instance.new("TextLabel", bg)
                txt.Size = UDim2.new(1, 0, 1, 0) txt.BackgroundTransparency = 1 txt.Text = p.Name .. "\n[" .. dist .. "s]"
                txt.TextColor3 = c txt.Font = Enum.Font.GothamBold txt.TextScaled = true
                local stroke = Instance.new("TextStroke", txt) stroke.Color = Color3.new(0,0,0) stroke.Thickness = 1
            end
        end
    end
end)
...
RecBtn.MouseButton1Click:Connect(function()
    ...
    ApplyToggleStyle(RecBtn, recording, Color3.fromRGB(255, 50, 50))
    RecBtn.Text = "RECORD REPLAY: " .. (recording and "RECORDING..." or "OFF")
    if recording then replayData = {} end
end)

PlayRecBtn.MouseButton1Click:Connect(function()
    if recording or #replayData == 0 then return end
    playing = not playing
    ApplyToggleStyle(PlayRecBtn, playing, Color3.fromRGB(0, 255, 50))
    PlayRecBtn.Text = "WATCH REPLAY: " .. (playing and "PLAYING..." or "OFF")
    if playing then playIndex = 1 end
end)

ClearRecBtn.MouseButton1Click:Connect(function()
    if not recording and not playing then replayData = {} ClearRecBtn.Text = "CLEARED!" task.delay(1, function() ClearRecBtn.Text = "CLEAR RECORDING" end) end
end)
...
RunService.Heartbeat:Connect(function()
    ...
    if not hrp or not hum or hum.Health <= 0 then
        if recording then RecBtn.Text = "RECORD REPLAY: OFF" ApplyToggleStyle(RecBtn, false, Color3.fromRGB(255, 50, 50)) recording = false end
        if playing then PlayRecBtn.Text = "WATCH REPLAY: OFF" ApplyToggleStyle(PlayRecBtn, false, Color3.fromRGB(0, 255, 50)) playing = false end
        return
    end

    if recording then table.insert(replayData, hrp.CFrame) end
    if playing then
        if playIndex <= #replayData then
            hrp.CFrame = replayData[playIndex]
            hrp.Velocity = Vector3.zero
            playIndex = playIndex + 1
        else
            playing = false
            PlayRecBtn.Text = "WATCH REPLAY: OFF" ApplyToggleStyle(PlayRecBtn, false, Color3.fromRGB(0, 255, 50))
        end
    end
end)

-- CHARACTER & FLIGHT SETTERS
local function SetUltraRun(state) ultraRunActive = state ApplyToggleStyle(UltraRunBtn, ultraRunActive, Color3.fromRGB(255, 80, 0)) UltraRunBtn.Text = "ULTRA RUN: " .. (ultraRunActive and "ON" or "OFF") if not state then local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") if hum then for _, t in pairs(hum:GetPlayingAnimationTracks()) do t:AdjustSpeed(1) end end end end
local function SetNoclip(state) noclipActive = state ApplyToggleStyle(NoclipBtn, noclipActive, Color3.fromRGB(0, 255, 200)) NoclipBtn.Text = "NOCLIP: " .. (noclipActive and "ON" or "OFF") end
local function SetUndie(state) undieActive = state ApplyToggleStyle(UndieBtn, undieActive, Color3.fromRGB(200, 50, 50)) UndieBtn.Text = "UN-DIE: " .. (undieActive and "ON" or "OFF") end
local function SetUnvoid(state) unvoidActive = state ApplyToggleStyle(UnvoidBtn, unvoidActive, Color3.fromRGB(50, 50, 255)) UnvoidBtn.Text = "UN-VOID: " .. (unvoidActive and "ON" or "OFF") end
local function SetChaosLag(state) infStabActive = state ApplyToggleStyle(InfStabBtn, infStabActive, Color3.fromRGB(255, 150, 0)) InfStabBtn.Text = "CHAOS LAG: " .. (infStabActive and "ON" or "OFF") if not state then local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") if hrp then hrp.Anchored = false hrp.Velocity = Vector3.zero end end end
local function SetCFSpeed(state) cfSpeedActive = state ApplyToggleStyle(CFrameSpeedBtn, cfSpeedActive, Color3.fromRGB(255, 100, 0)) CFrameSpeedBtn.Text = "CF SPD: " .. (cfSpeedActive and "ON" or "OFF") end
local function SetSpin(state) spinActive = state ApplyToggleStyle(SpinBtn, spinActive, Color3.fromRGB(0, 255, 50)) SpinBtn.Text = "SPIN: " .. (spinActive and "ON" or "OFF") end
...
GenSaveBtn.MouseButton1Click:Connect(function()
    local tgs = string.format("%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d",
        ...
        cfSpeedActive and 1 or 0, spinActive and 1 or 0, fpsActive and 1 or 0,
        pingActive and 1 or 0, aimbotActive and 1 or 0, unafkActive and 1 or 0
    )
    local tabsOrderStr = ""
    ...
    local rawStr = string.format("%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s", currentTheme, ShrinkBox.Text, 30, WsBox.Text, JpBox.Text, GravBox.Text, CFrameSpeedBox.Text, SpinBox.Text, tgs, tabsOrderStr, lagStr)
...
LoadSaveBtn.MouseButton1Click:Connect(function()
    ...
    if #p >= 11 then
        SetTheme(p[1]) ShrinkBox.Text = p[2] ApplyShrink() WsBox.Text = p[4] applyWS() JpBox.Text = p[5] applyJP() GravBox.Text = p[6] applyGrav() CFrameSpeedBox.Text = p[7] SpinBox.Text = p[8]
        local t = p[9]
        SetESP(t:sub(1,1)=="1") SetUltraRun(t:sub(2,2)=="1") SetNoclip(t:sub(3,3)=="1") ToggleInvis(t:sub(4,4)=="1") SetUndie(t:sub(5,5)=="1") SetFly(t:sub(6,6)=="1") TogglePlatform(t:sub(7,7)=="1") SetUnvoid(t:sub(8,8)=="1") SetChaosLag(t:sub(9,9)=="1") SetCFSpeed(t:sub(10,10)=="1") SetSpin(t:sub(11,11)=="1") SetFPS(t:sub(12,12)=="1") SetPing(t:sub(13,13)=="1") SetAimbot(t:sub(14,14)=="1") SetUnAFK(t:sub(15,15)=="1")
...
local infTpConn = nil
local colorPalette = {Color3.fromRGB(0,255,255), Color3.fromRGB(255,0,0), Color3.fromRGB(0,255,0), Color3.fromRGB(255,255,0), Color3.fromRGB(255,0,255)}
    ...
    infTpConn = RunService.Heartbeat:Connect(function()
        ...
        if typeof(currentInfTpTarget) == "Instance" then
            ...
            if tChar and tChar.Parent then
                ...
                if tHrp and tHum and tHum.Health > 0 then tCFrame = tHrp.CFrame * CFrame.new(0, 0, 3) val = true end
            end
        elseif type(currentInfTpTarget) == "table" then
            if currentInfTpTarget.part and currentInfTpTarget.part.Parent then tCFrame = currentInfTpTarget.part.CFrame * CFrame.new(0, 3, 0) val = true
            else tCFrame = CFrame.new(currentInfTpTarget.pos + Vector3.new(0, 3, 0)) val = true end
        end
        local mHrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if val and tCFrame and mHrp then mHrp.CFrame = tCFrame else stopInfTp() updatePlayerList() end
    end)
end
...
updatePlayerList = function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            ...
            local btn = Instance.new("TextButton", row) btn.Size = UDim2.new(0.85, 0, 1, 0)
            if listMode == "INF TP" then
                local isActive = (currentInfTpTarget == p) btn.Text = p.DisplayName .. (isActive and " [ON]" or "[OFF]") ApplyStyle(btn, isActive and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0))
            else btn.Text = p.DisplayName ApplyStyle(btn, GetPlayerColor(p)) end
            
            local cBtn = Instance.new("TextButton", row) cBtn.Size = UDim2.new(0.13, 0, 1, 0) cBtn.Position = UDim2.new(0.87, 0, 0, 0) cBtn.Text = "🎨" ApplyStyle(cBtn, GetPlayerColor(p))
            
            btn.MouseButton1Click:Connect(function()
                if listMode == "TP" then local pChar = p.Character if pChar and pChar:FindFirstChild("HumanoidRootPart") then LocalPlayer.Character.HumanoidRootPart.CFrame = pChar.HumanoidRootPart.CFrame end
                elseif listMode == "VIEW" then if p.Character and p.Character:FindFirstChild("Humanoid") then workspace.CurrentCamera.CameraSubject = p.Character.Humanoid end
                elseif listMode == "INF TP" then if currentInfTpTarget == p then stopInfTp() else startInfTp(p) end updatePlayerList() end
            end)
            cBtn.MouseButton1Click:Connect(function()
                local cId = 1 for i, v in ipairs(colorPalette) do if v == GetPlayerColor(p) then cId = i break end end
                playerColors[p.Name] = colorPalette[cId % #colorPalette + 1] updatePlayerList()
            end)
        end
    end
    if listMode == "TP" or listMode == "VIEW" or listMode == "INF TP" then
        for i, spot in ipairs(savedSpots) do
            local row = MakeRow(PlayerList)
            local tpBtn = Instance.new("TextButton", row) tpBtn.Size = UDim2.new(0.75, 0, 1, 0)
...
mouse.Button1Down:Connect(function() if waitingForClick then waitingForClick = false AddTpBtn.Text = "ADD CUSTOM TP PART" table.insert(savedSpots, {name = mouse.Target and ("[" .. mouse.Target.Name .. "]") or "SPOT", part = mouse.Target, pos = mouse.Hit.Position}) updatePlayerList() end end)
...
RunService.Stepped:Connect(function()
    if invisActive and realChar and fakeChar then
        ...
        local fakeTool, realHum = fakeChar:FindFirstChildOfClass("Tool"), realChar:FindFirstChildOfClass("Humanoid")
...
UIS.InputBegan:Connect(function(input, gpe) if invisActive and realChar and fakeChar and input.UserInputType == Enum.UserInputType.MouseButton1 and not gpe then isStriking = true local rHrp, fHrp = realChar:FindFirstChild("HumanoidRootPart"), fakeChar:FindFirstChild("HumanoidRootPart") if rHrp and fHrp then rHrp.CFrame = fHrp.CFrame end task.delay(0.1, function() isStriking = false end) end end)
...
TogglePlatform = function(state)
    ...
    if platActive and hrp then
        ...
        platConn = RunService.RenderStepped:Connect(function()
            ...
            if currentY - (cHrp.Position.Y - 3.5) > 1.5 and not (platDownPressed or UIS:IsKeyDown(Enum.KeyCode.LeftControl)) then
                ...
                if platFails > 8 then TogglePlatform(false) end
            end
        end)
...
local flyConn, upPressed, downPressed = nil, false, false
SetFly = function(state)
    ...
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") if not hrp or not hum then return end
    if flying then
        ...
        flyConn = RunService.RenderStepped:Connect(function()
            ...
            if UIS:IsKeyDown(Enum.KeyCode.Space) or upPressed then yMove = spd end if UIS:IsKeyDown(Enum.KeyCode.LeftControl) or downPressed then yMove = -spd end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    else if lagState == "LAG" then hrp.Velocity = Vector3.zero hrp.RotVelocity = Vector3.zero hrp.Anchored = true else hrp.Anchored = false end end
end)
...
UIS.InputBegan:Connect(function(input) lastActive = tick() end)
...
local function ForceCleanup()
    SetESP(false) SetUltraRun(false) SetNoclip(false) SetChaosLag(false) SetSpin(false) SetCFSpeed(false) SetFly(false) TogglePlatform(false) ToggleInvis(false) SetRadar(false) SetAimbot(false) SetFPS(false) SetPing(false) SetUnAFK(false)
...
task.spawn(function()
    while task.wait(1) do
        ...
        if tick() - lastActive > (tonumber(AfkBox.Text) or 30) then ForceCleanup() break end
    end
end)
...
CloseBtn.MouseButton1Click:Connect(function() ForceCleanup() end)
