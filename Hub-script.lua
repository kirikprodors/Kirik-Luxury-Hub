...
FlyBtn = Instance.new("TextButton", FlyRow) 
FlyBtn.Size = UDim2.new(0.65, 0, 1, 0) 
FlyBtn.Text = "FLY: OFF" 
ApplyStyle(FlyBtn, Color3.fromRGB(255, 0, 255)) 

FlySpeedBox = Instance.new("TextBox", FlyRow) 
...
-- ==================== FLOATING MOBILE UI ==================== 
FlyUI = Instance.new("Frame", ScreenGui) 
...
PlatUI = Instance.new("Frame", ScreenGui) 
...
SetFly = function(state) 
    if flying == state then return end 
    flying = state 
    ...
    infTpConn = RunService.Heartbeat:Connect(function() 
        ...
        if typeof(currentInfTpTarget) == "Instance" then 
            ...
            if charObj and charObj.Parent then 
                ...
                if tH and tHu and tHu.Health > 0 then 
                    tCF = tH.CFrame * CFrame.new(0, 0, 3) 
                    valid = true 
                end 
            end 
        elseif type(currentInfTpTarget) == "table" then 
            if currentInfTpTarget.part and currentInfTpTarget.part.Parent then 
                tCF = currentInfTpTarget.part.CFrame * CFrame.new(0, 3, 0) 
                valid = true 
            else 
                tCF = CFrame.new(currentInfTpTarget.pos + Vector3.new(0, 3, 0)) 
                valid = true 
            end 
        end 
...
updatePlayerList = function() 
    ...
    for _, player in pairs(Players:GetPlayers()) do 
        if player ~= LocalPlayer then 
            ...
            if listMode == "INF TP" then 
                local act = (currentInfTpTarget == player) 
                btn.Text = player.DisplayName .. (act and " [ON]" or " [OFF]") 
                ApplyStyle(btn, act and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)) 
            else 
    ...
    if listMode == "TP" or listMode == "VIEW" or listMode == "INF TP" then 
        for i, spot in ipairs(savedSpots) do 
            ...
            local act = (currentInfTpTarget == spot) 
            tpBtn.Text = spot.name .. (act and " [ON]" or " [OFF]") 
            ApplyStyle(tpBtn, act and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(0, 255, 100)) 
            ...
            tpBtn.MouseButton1Click:Connect(function() 
                if listMode == "TP" then 
                    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") 
                    if hrp then 
                        hrp.CFrame = spot.part and spot.part.Parent and (spot.part.CFrame * CFrame.new(0, 3, 0)) or CFrame.new(spot.pos + Vector3.new(0, 3, 0)) 
                    end 
                elseif listMode == "VIEW" then 
...
updateNpcList = function() 
    ...
    for _, npc in ipairs(cachedNPCs) do 
        if npc and npc.Parent then 
            ...
            local btn = Instance.new("TextButton", row) 
            btn.Size = UDim2.new(1, 0, 1, 0) 
            local act = (currentInfTpTarget == npc) 
            btn.Text = npc.Name .. (act and " [ON]" or " [OFF]") 
            ApplyStyle(btn, act and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 100, 0)) 
...
updateLagList = function() 
    ...
    for i, preset in ipairs(lagChain) do 
        ...
        local aBox = Instance.new("TextBox", row) 
        aBox.Size = UDim2.new(0.38, 0, 1, 0) 
        aBox.Text = tostring(preset.anchor) 
        ApplyStyle(aBox, Color3.fromRGB(255, 0, 150)) 
        
        local fBox = Instance.new("TextBox", row) 
        fBox.Size = UDim2.new(0.38, 0, 1, 0) 
        fBox.Position = UDim2.new(0.42, 0, 0, 0) 
        fBox.Text = tostring(preset.free) 
        ApplyStyle(fBox, Color3.fromRGB(0, 255, 150)) 
        
        local dBtn = Instance.new("TextButton", row) 
        dBtn.Size = UDim2.new(0.16, 0, 1, 0) 
        dBtn.Position = UDim2.new(0.84, 0, 0, 0) 
        dBtn.Text = "-" 
        ApplyStyle(dBtn, Color3.fromRGB(255, 0, 0)) 
        
        aBox.FocusLost:Connect(function() 
            preset.anchor = math.max(0, tonumber(aBox.Text) or preset.anchor) 
            aBox.Text = tostring(preset.anchor) 
        end) 
        
        fBox.FocusLost:Connect(function() 
            preset.free = math.max(0, tonumber(fBox.Text) or preset.free) 
            fBox.Text = tostring(preset.free) 
        end) 
...
ModeBtn.MouseButton1Click:Connect(function() 
    listMode = (listMode == "TP" and "VIEW" or (listMode == "VIEW" and "INF TP" or "TP")) 
    ModeBtn.Text = "LIST MODE: " .. listMode 
    updatePlayerList() 
end) 

NpcModeBtn.MouseButton1Click:Connect(function() 
    npcListMode = (npcListMode == "TP" and "VIEW" or (npcListMode == "VIEW" and "INF TP" or "TP")) 
    NpcModeBtn.Text = "LIST MODE: " .. npcListMode 
    updateNpcList() 
end) 

NpcRefreshBtn.MouseButton1Click:Connect(function() 
    cachedNPCs = {} 
    NpcRefreshBtn.Text = "SCANNING..." 
    task.wait(0.1) 
    for _, obj in ipairs(workspace:GetDescendants()) do 
        if obj:IsA("Model") and obj ~= LocalPlayer.Character and obj:FindFirstChildOfClass("Humanoid") and obj:FindFirstChild("HumanoidRootPart") and not Players:GetPlayerFromCharacter(obj) then 
...
GenSaveBtn.MouseButton1Click:Connect(function() 
    local tgs = string.format("%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d", 
        espActive and 1 or 0, ultraRunActive and 1 or 0, noclipActive and 1 or 0, invisActive and 1 or 0, 
        undieActive and 1 or 0, flying and 1 or 0, platActive and 1 or 0, unvoidActive and 1 or 0, 
        infStabActive and 1 or 0, cfSpeedActive and 1 or 0, spinActive and 1 or 0, fpsActive and 1 or 0, 
        pingActive and 1 or 0, aimbotActive and 1 or 0, antiAfkActive and 1 or 0) 
    ...
    local rawStr = string.format("%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s", 
        currentTheme, ShrinkBox.Text, AfkBox.Text, WsBox.Text, JpBox.Text, GravBox.Text, CFrameSpeedBox.Text, SpinBox.Text, tgs, tabsOrderStr, lagStr) 
...
AddServiceConn(RunService.Stepped:Connect(function() 
    ...
    if invisActive and realChar and fakeChar then
        local rHrp = realChar:FindFirstChild("HumanoidRootPart")
        local fHrp = fakeChar:FindFirstChild("HumanoidRootPart")
        if rHrp and fHrp and not isStriking then 
            rHrp.CFrame = fHrp.CFrame * CFrame.new(0, 500, 0) 
            rHrp.Velocity = Vector3.zero 
            rHrp.RotVelocity = Vector3.zero 
        end
        local fTool = fakeChar:FindFirstChildOfClass("Tool")
        local rHum = realChar:FindFirstChildOfClass("Humanoid")
        if fTool and rHum then 
            local rTool = LocalPlayer.Backpack:FindFirstChild(fTool.Name) 
            if rTool then rHum:EquipTool(rTool) end 
        elseif not fTool and rHum then 
            rHum:UnequipTools() 
        end
    end
end))
...
local function ForceCleanup() 
    ...
    if PlatUI then PlatUI.Visible = false end 
    stopInfTp() 
