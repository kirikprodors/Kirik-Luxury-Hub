...
-- ==================== FORWARD DECLARATIONS (STRICT SCOPING) ====================
...
local MainFrame, Sidebar, TabContainer, SearchBox, SideLayout, StatsLayout
...
-- ==================== INITIALIZE GUI & PARENTING ====================
local currentTheme = "NEON"
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "KirikLuxuryHub"
ScreenGui.ResetOnSpawn = false

-- Assigning Parent FIRST to prevent TweenService silent crashes
local function AssignParent(gui)
...
InfJumpBtn = Instance.new("TextButton", MakeRow(CharScroll)) 
InfJumpBtn.Size = UDim2.new(1, 0, 1, 0) 
InfJumpBtn.Text = "INFINITE JUMP: OFF" 
ApplyStyle(InfJumpBtn, Color3.fromRGB(0, 255, 150))

ClickTpBtn = Instance.new("TextButton", MakeRow(CharScroll)) 
ClickTpBtn.Size = UDim2.new(1, 0, 1, 0) 
ClickTpBtn.Text = "CLICK TELEPORT: OFF" 
ApplyStyle(ClickTpBtn, Color3.fromRGB(255, 150, 0))
    ...
    infTpConn = RunService.Heartbeat:Connect(function()
        ...
        if typeof(currentInfTpTarget) == "Instance" then
            ...
            if charObj and charObj.Parent then
                local tH = charObj:FindFirstChild("HumanoidRootPart")
                local tHu = charObj:FindFirstChildOfClass("Humanoid")
        ...
        elseif type(currentInfTpTarget) == "table" then
            if currentInfTpTarget.part and currentInfTpTarget.part.Parent then 
                tCF = currentInfTpTarget.part.CFrame * CFrame.new(0, 3, 0) valid = true
            else 
                tCF = CFrame.new(currentInfTpTarget.pos + Vector3.new(0, 3, 0)) valid = true 
            end
        end
    ...
    if listMode == "TP" or listMode == "VIEW" or listMode == "INF TP" then
        for i, spot in ipairs(savedSpots) do
            ...
            delBtn.MouseButton1Click:Connect(function() 
                ...
                table.remove(savedSpots, i) updatePlayerList() 
            end)
        end
    end
    PerformSearch()
        ...
        if npc and npc.Parent then
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                elseif npcListMode == "INF TP" then 
                    ...
                    updateNpcList() updatePlayerList() 
                end
            end)
        end
    end
...
updateLagList = function()
    ...
    for i, preset in ipairs(lagChain) do
        ...
        dBtn.MouseButton1Click:Connect(function() 
            table.remove(lagChain, i) updateLagList() 
        end)
    end
    PerformSearch()
end
...
ApplyShrink = function()
    ...
    if factor and factor > 0 then 
        MainScaler.Scale = 1/factor FlyScaler.Scale = 1/factor PlatScaler.Scale = 1/factor
    else 
        ShrinkBox.Text = "1" MainScaler.Scale = 1 FlyScaler.Scale = 1 PlatScaler.Scale = 1 
    end
end
